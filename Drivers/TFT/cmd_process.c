...
float Lp_Q_Value = 0.801f,
	Hp_Q_Value = 0.801f;
my_Filter my_Filter1;
/*! 
 *  \brief  消息处理流程
 *  \param msg 待处理消息
 *  \param size 消息长度
...
{
		uchar buffer[12] = {0};
		snprintf((char *)buffer,sizeof(buffer),"%.3f",value);//把浮点数转换为字符串(保留一位小数)
		SetTextValue(screen_id,control_id,buffer);
}
...
void NotifyButton(uint16 screen_id, uint16 control_id, uint8  state,void* userdata)
{
   if(screen_id == 0 && control_id == 5)  
   {
		   ...
		   my_Filter1.Mode = Sweep;	//扫频模式为1
	 
   }
   
   if(screen_id == 0 && control_id == 10)
	{
		//wifi显示幅频特性曲线
	}
	...
	{
		my_Filter1.Mode = sysReset;
	}
//	 	SetControlVisiable(0,12,1);   //显示OK
//		delay_ms(500);
//		SetControlVisiable(0,12,0);   //不显示OK
...
{ 
	   if(screen_id == 1 && control_id == 2)  //低通Q值校准
   {
		Lp_Q_Value = atof((const char*)str);
   }
   if(screen_id == 1 && control_id == 9)
   {
		Hp_Q_Value = atof((const char*)str);
   }
   MAX262_Init();
   SetTextValueFloat(1,6,Lp_Q_Value);	//更新显示Q值
   SetTextValueFloat(1,10,Hp_Q_Value);	//更新显示Q值
}
...
{
	if(screen_id == 0 && control_id == 4)  //选择截止频率
		my_Filter1.cutoff_fre = item * 1000;
	 if(screen_id == 0 && control_id == 8)  //选择滤波器类型
	 {
			my_Filter1.filterKind = item;   //0-低通， 1-高通，2-椭圆，3放大
		  my_Filter1.Mode = Normal;	//扫频模式为1
	 }
	 if(screen_id == 0 && control_id == 17)
		 my_Filter1.amp_dB = item;
//	 	SetControlVisiable(0,12,1);   //显示OK
//		delay_ms(500);
//		SetControlVisiable(0,12,0);   //不显示OK
}
